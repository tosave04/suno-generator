...
# 🤖 Copilot Instructions — Suno Generator Application
...
5. **Anti-crash token LLM** : JAMAIS créer/modifier plus de **1-2 fichiers par étape**. Toute tâche impliquant 3+ fichiers DOIT être découpée en étapes séquentielles avec validation intermédiaire. Un crash LLM = perte totale du travail en cours.
...
> ⚠️ **RÈGLE ABSOLUE** : Le dépassement de la limite de tokens LLM provoque un **crash irrécupérable** (perte totale de la réponse en cours). C'est la cause n°1 d'échec. Chaque étape DOIT être courte et auto-suffisante.

### Principe : Découpage OBLIGATOIRE étape par étape

**❌ INTERDIT — PROVOQUE UN CRASH** :

```
1. Créer 10 composants + routes + actions + tests d'un coup
   → CRASH GARANTI : error llm length exceeded, TOUT le travail est perdu
...
**✅ OBLIGATOIRE — Progression itérative** :

```
1. Créer schéma Zod + types TypeScript (1-2 fichiers max)
2. Valider build (`yarn lint; yarn build`)
3. Créer UNE action serveur avec validation Zod
4. Créer UN composant UI, valider
5. Créer le composant suivant, valider
6. Intégrer routes Next.js
7. Validation finale
```
